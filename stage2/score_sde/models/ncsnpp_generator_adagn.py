...
class NCSNpp(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.all_resolutions = all_resolutions = [128, 64, 32, 16]
        ...
        in_channels = config.num_input_channels # added
        if progressive_input != 'none':
            input_pyramid_ch = in_channels # modified：channels -> in_channels
...
class WaveletNCSNpp(NCSNpp):
    ...
    def __init__(self, config):
        ...
        # Downsampling block
        channels = config.num_channels * self.patch_size**2
        in_channels = config.num_input_channels # modified by gx
        if progressive_input != 'none':
            # input_pyramid_ch = channels
            input_pyramid_ch = in_channels # modified by gx：channels -> in_channels

        # modules.append(conv3x3(channels, nf))
        modules.append(conv3x3(in_channels, nf)) # modified by gx：channels -> in_channels
        hs_c = [nf]
        hs_c2 = [nf]
