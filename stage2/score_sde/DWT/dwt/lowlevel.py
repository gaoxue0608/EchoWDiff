...
def afb1d(x, h0, h1, mode='zero', dim=-1):
    ...
    if mode == 'per' or mode == 'periodization':
        ...
        if mode == 'zero':
            ...
            h = h.to(x.device) # modified by gx
...
class AFB2D(Function):
    ...
    def backward(ctx, low, highs):
        ...
        if ctx.needs_input_grad[0]:
            ...
            h0_row = h0_row.to(low.device) # modified by gx
            h1_row = h1_row.to(low.device) # modified by gx
            h0_col = h0_col.to(low.device) # modified by gx
            h1_col = h1_col.to(low.device) # modified by gx
